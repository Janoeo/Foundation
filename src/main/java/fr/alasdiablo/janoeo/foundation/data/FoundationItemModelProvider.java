...
public class FoundationItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        List<String> dusts = Arrays.asList(
                ALUMINIUM_DUST, AMETHYST_DUST, COAL_DUST, COPPER_DUST, DIAMOND_DUST, EMERALD_DUST, GOLD_DUST, IRON_DUST,
                LAPIS_DUST, LEAD_DUST, NICKEL_DUST, SILVER_DUST, TIN_DUST, URANIUM_DUST
        );
        this.registerDustItem(dusts);

        List<String> nuggets = Arrays.asList(
                ALUMINIUM_NUGGET, COAL_NUGGET, COPPER_NUGGET, DIAMOND_NUGGET, EMERALD_NUGGET, LAPIS_NUGGET, LEAD_NUGGET,
                NICKEL_NUGGET, REDSTONE_NUGGET, SILVER_NUGGET, TIN_NUGGET, URANIUM_NUGGET
        );
        this.registerNuggetItem(nuggets);

        List<String> ingots = Arrays.asList(
                ALUMINIUM_INGOT, LEAD_INGOT, NICKEL_INGOT, SILVER_INGOT, TIN_INGOT, URANIUM_INGOT
        );
        this.registerIngotItem(ingots);

        List<String> raws = Arrays.asList(
                RAW_ALUMINIUM, RAW_LEAD, RAW_NICKEL, RAW_SILVER, RAW_TIN, RAW_URANIUM
        );
    ...
    private void registerDustItem(List<String> dusts) {
        dusts.forEach(dust -> withExistingParent(dust, GENERATED)
                .texture("layer0", new ResourceLocation(MOD_ID, "item/dust/" + dust)));
    }

    private void registerNuggetItem(List<String> nuggets) {
        nuggets.forEach(nugget -> withExistingParent(nugget, GENERATED)
                .texture("layer0", new ResourceLocation(MOD_ID, "item/nugget/" + nugget)));
    }

    private void registerIngotItem(List<String> ingots) {
        ingots.forEach(ingot -> withExistingParent(ingot, GENERATED)
                .texture("layer0", new ResourceLocation(MOD_ID, "item/ingot/" + ingot)));
    }

    private void registerRawItem(List<String> raws) {
        raws.forEach(raw -> withExistingParent(raw, GENERATED)
                .texture("layer0", new ResourceLocation(MOD_ID, "item/raw/" + raw)));
    }
